...
public class Player {
    ...
    private final double velocity = 18;
    ...
    public Vector getDirVector() {
        return new Vector(Math.cos(Math.toRadians(angle)), Math.sin(Math.toRadians(angle)));
    }

    public Vector getStandardDirVector() {
        return new Vector(Math.cos(Math.toRadians(360 - angle)), Math.sin(Math.toRadians(360 - angle)));
    }
    ...
    public double angleToEnemy(Soldier soldier) {
        Vector playerVector = getDirVector();
        Vector enemyVector = new Vector(soldier.getX() - xCoor, -(soldier.getY() - yCoor));

        double dotProduct = playerVector.x() * enemyVector.x() + playerVector.y() * enemyVector.y();
    ...
    private void checkHit() {
        for (Soldier enemy :
            ...
            Vector playerEnemyVector = new Vector(enemy.getX() - xCoor, enemy.getY() - yCoor);


            //tan angle = deltaY/deltaX
            double angle = Vector.angleToXAxis(playerEnemyVector) - (360 - this.angle);
